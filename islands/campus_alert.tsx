...
export default function Home({ links }: HomeType) {
    ...
    const [lastCharacterAlertCampus, setLastCharacterAlertCampus] = useState<string | null>(null);
    const [alertCampuses, setAlertCampuses] = useState<string[]>([]);
    useEffect(() => {
        if (searchMenu === "") {
            setMenu(links);
        } else {
                ...
                res.json().then((resData) => {
                    for (
                        ...
                        if (
                            ...
                            const warning_data: JSX.Element[] = [];
                                ...
                                if (
                                    ...
                                    if (transweather[warning["code"]].endsWith("特別警報")) {
                                        ...
                                        if (currentHour >= 7 && currentHour <= 10) {
                                            setAlertCampuses((prevAlertCampuses) => {
                                                const newAlertCampuses = [...prevAlertCampuses];
                                                const campusName = favoriteCampus[i]["name"] + "キャンパス";
                                                if (!newAlertCampuses.includes(campusName)) {
                                                    newAlertCampuses.push(campusName);
                                                }
                                                return newAlertCampuses;
                                            });
                                        }
    ...
    return (
        <>
            <div class="bg-gray-100">
                {alertCampuses.length > 0 && (
                    <div class="bg-red-500 text-white text-center py-2">
                        <p>午前7時時点で警報発令中のため</p>
                        <p class="text-xl font-bold">
                            {alertCampuses.join(", ")}は休校の可能性があります。
                        </p>
                        <p>キャンパスのSlackチャンネルを参照し、指示に従ってください。</p>
                    </div>
                )}
                    ...
                    <div>
                        <div class="my-3">
                            ...
                            <div class="flex item-center">
                                <p class="font-bold text-gray-600 text-sm">
                                    <button
                                        class="bg-green-500 text-white rounded-full w-5 h-5 hover:bg-green-600 duration-500 m-1">
                                        ＋
                                    </button>
